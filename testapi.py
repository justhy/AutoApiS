...
config_list = {'每次轮数':6,
	       ...
	       '是否开启随机api顺序':'Y',
	       '是否开启各api延时':'N','分延时范围开始':5,'分结束':10,
	       '是否开启备用应用':'N','是否开启测试':'N'}
log_list=[0]*len(id_list)
num1 = [0]*len(id_list)
...
def sendTgBot():
    if not ('tg_bot' in locals() or 'tg_bot' in globals()) or len(tg_bot) != 2:
        exit(0)
    content = "AutoApiSuper简报：" + "\n"
    for i in range(0, len(id_list)):
        content=content+'账号 '+str(i+1)+' ：成功 '+str(len(randomapi)*config_list['每次轮数']-log_list[i])+' 次，失败 '+str(log_list[i])+' 次'+'\n'
    headers={
    ...
    for retry_ in range(4):  
        posttext=req.post(r'https://api.telegram.org/bot'+tg_bot[1]+r'/sendMessage',headers=headers,data=json.dumps(data))
